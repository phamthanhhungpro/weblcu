...
namespace WebApp.Areas.Admin.Controllers
{
    ...
    public class FestivalController(LogService logService, ILogger<FestivalController> logger, 
        AuthenUtils authenUtils, IWebHostEnvironment environment, FestivalService FestivalService, PeopleService peopleService) : BaseController<FestivalController>(logger, authenUtils,logService)
    {
        private IWebHostEnvironment _hostEnvironment = environment;
        private FestivalService _FestivalService = FestivalService;
        ...
        public ActionResult Index()
        {
            ...
            if (CheckFunctionPermission(Constants.PERMISSION_FESTIVAL_VIEW))
            {
                permission.IsView = true;
                permission.IsAdd = CheckFunctionPermission(Constants.PERMISSION_FESTIVAL_ADD);
                permission.IsEdit = CheckFunctionPermission(Constants.PERMISSION_FESTIVAL_EDIT);
                permission.IsDelete = CheckFunctionPermission(Constants.PERMISSION_FESTIVAL_DELETE);
                permission.IsShow = CheckFunctionPermission(Constants.PERMISSION_FESTIVAL_SHOW);
                ViewBag.Permission = permission;
                ViewBag.FestivalCategories = GetListFestivalCategory();
        ...
        {
            if (CheckFunctionPermission(Constants.PERMISSION_FESTIVAL_ADD))
            {
                var permission = new FestivalPermission();
                permission.IsView = true;
                permission.IsAdd = CheckFunctionPermission(Constants.PERMISSION_FESTIVAL_ADD);
                ...
                permission.IsShow = CheckFunctionPermission(Constants.PERMISSION_FESTIVAL_SHOW);
                ViewBag.Permission = permission;
                var excel = new GenericExcelHandler<FestivalModel>(Log, _hostEnvironment.ContentRootPath, UserData.Id);
                excel.Read();
        ...
        private List<FestivalModel> ReadExcelSheet(string filePath, bool isSave = false)
        {
            var countAdd = 0;
                ...
                using (SpreadsheetDocument doc = SpreadsheetDocument.Open(fs, false))
                {
                    ...
                    for (int i = 1; i < rows.Count(); i++)
                    {
                        ...
                        if (isAdd)
                        {
                            if (isSave)
                            {
                                if (_FestivalService.AddByCheckName(model).IsSuccess())
                                    countAdd++;
                                else
                                {
                                    SetBaseValue(model);
        ...
        private void SetBaseValue(FestivalModel model)
        {
        ...
        private void SetData(FestivalModel FestivalModel, int index, Cell cellData, SharedStringTable sst)
        {
        ...
        public JsonResult LoadAjaxNew(string category,string people)
        {
            ...
            IEnumerable<Festival> list = null;
            if (term != null)
            {
                term = term.ToLower();
            }
            ...
            if (CheckFunctionPermission(Constants.PERMISSION_FESTIVAL_VIEW))
            {
                Expression<Func<Festival, bool>> expression = o => o.DeleteStatus == Enums.DeleteStatus.Normal;
                ...
                list = _FestivalService.GetAll(expression);
        ...
        private Object BindData(IEnumerable<Festival> list, int start, int length)
        {
        ...
        private List<SelectListItem> GetListFestivalCategory(int? FestivalCategoryId = null)
        {
        ...
        [HttpPost]
        [Route("Create")]
        public ActionResult Create(FestivalModel model)
        {
            if (CheckFunctionPermission(Constants.PERMISSION_FESTIVAL_ADD))
            {
                if (ModelState.IsValid)
                {
                    _FestivalService.Add(model);
        ...
        {
            if (CheckFunctionPermission(Constants.PERMISSION_FESTIVAL_EDIT))
            {
                var data = _FestivalService.GetById(id);
        ...
        public ActionResult Edit(FestivalModel model)
        {
            if (CheckFunctionPermission(Constants.PERMISSION_FESTIVAL_EDIT))
            {
                if (ModelState.IsValid)
                {
                    var result = _FestivalService.Update(model);
                    if (result.IsSuccess())
                    {
        ...
        [HttpPost]
        [Route("EditExcel")]
        public ActionResult EditExcel(FestivalModel model, int idExcel)
        {
            if (CheckFunctionPermission(Constants.PERMISSION_FESTIVAL_EDIT))
            {
                if (ModelState.IsValid)
                {
                    model.Id = 0;
                    var result = _FestivalService.Add(model);
                    if (result.IsSuccess())
                    {
                        var excel = new GenericExcelHandler<FestivalModel>(Log, _hostEnvironment.ContentRootPath, UserData.Id);
                        excel.Read();
        ...
        {
            MessageResult message = new MessageResult();
            if (CheckFunctionPermission(Constants.PERMISSION_FESTIVAL_DELETE))
            {
                message = new MessageResult();
                message = _FestivalService.Delete(id);
