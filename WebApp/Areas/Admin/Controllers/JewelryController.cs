...
namespace WebApp.Areas.Admin.Controllers
{
    ...
    public class JewelryController(LogService logService, ILogger<JewelryController> logger, AuthenUtils authenUtils, IWebHostEnvironment environment, JewelryCategoryService JewelryCategoryService, JewelryService JewelryService, PeopleService peopleService) : BaseController<JewelryController>(logger, authenUtils, logService)
    {
        private IWebHostEnvironment _hostEnvironment = environment;
        private JewelryCategoryService _JewelryCategoryService = JewelryCategoryService;
        private JewelryService _JewelryService = JewelryService;
        ...
        {
            var permission = new JewelryPermission();
            if (CheckFunctionPermission(Constants.PERMISSION_JEWELRY_VIEW))
            {
                permission.IsView = true;
                permission.IsAdd = CheckFunctionPermission(Constants.PERMISSION_JEWELRY_ADD);
                permission.IsEdit = CheckFunctionPermission(Constants.PERMISSION_JEWELRY_EDIT);
                permission.IsDelete = CheckFunctionPermission(Constants.PERMISSION_JEWELRY_DELETE);
                permission.IsShow = CheckFunctionPermission(Constants.PERMISSION_JEWELRY_SHOW);
                ViewBag.Permission = permission;
                ViewBag.JewelryCategories = GetListJewelryCategory();
        ...
        {
            if (CheckFunctionPermission(Constants.PERMISSION_JEWELRY_ADD))
            {
                var permission = new JewelryPermission();
                permission.IsView = true;
                permission.IsAdd = CheckFunctionPermission(Constants.PERMISSION_JEWELRY_ADD);
                ...
                permission.IsShow = CheckFunctionPermission(Constants.PERMISSION_JEWELRY_SHOW);
                ViewBag.Permission = permission;
                var excel = new GenericExcelHandler<Jewelry>(Log, _hostEnvironment.ContentRootPath, UserData.Id);
                excel.Read();
        ...
        public ActionResult ImportExcel(IFormFile file, bool save = false)
        {
            if (CheckFunctionPermission(Constants.PERMISSION_JEWELRY_ADD))
            {
                ...
                permission.IsShow = CheckFunctionPermission(Constants.PERMISSION_JEWELRY_SHOW);
                ViewBag.Permission = permission;
                var excel = new GenericExcelHandler<JewelryModel>(Log, _hostEnvironment.ContentRootPath, UserData.Id);
                excel.Read();
        ...
        private List<JewelryModel> ReadExcelSheet(string filePath, bool isSave = false)
        {
            var countAdd = 0;
                ...
                using (SpreadsheetDocument doc = SpreadsheetDocument.Open(fs, false))
                {
                    ...
                    for (int i = 1; i < rows.Count(); i++)
                    {
                        ...
                        if (isAdd)
                        {
                            if (isSave)
                            {
                                if (_JewelryService.AddByCheckName(model).IsSuccess())
                                    countAdd++;
                                else
                                {
                                    SetBaseValue(model);
        ...
        private void SetBaseValue(JewelryModel model)
        {
            var type = model.GetCategoryName();
            if (!string.IsNullOrEmpty(type))
            {
                var typeData = _JewelryCategoryService.GetFirstOrDefault(o => o.Name.ToLower().Equals(type));
                if (typeData != null)
                {
        ...
        private void SetData(JewelryModel JewelryModel, int index, Cell cellData, SharedStringTable sst)
        {
        ...
        public JsonResult LoadAjaxNew(string category, string people)
        {
            ...
            if (CheckFunctionPermission(Constants.PERMISSION_JEWELRY_VIEW))
            {
                Expression<Func<Jewelry, bool>> expression = o => o.DeleteStatus == Enums.DeleteStatus.Normal;
                if (c > 0)
                {
                ...
                list = _JewelryService.GetAll(expression);
        ...
        private Object BindData(IEnumerable<Jewelry> list, int start, int length)
        {
        ...
        private List<SelectListItem> GetListJewelryCategory(int? JewelryCategoryId = null)
        {
            Expression<Func<JewelryCategory, bool>> query = x => x.Status == Enums.ActiveStatus.Active && x.Parent == null;
            if (JewelryCategoryId.HasValue)
            {
                query = ExtensionMethod.AndAlso(query, x => x.Id != JewelryCategoryId.Value);
            }
            var companyData = new List<SelectListItem>();
            foreach (var item in _JewelryCategoryService.GetAll().Where(query.Compile()))
            {
                companyData.AddRange(CreateJewelryCategory(item, JewelryCategoryId));
            }

            return companyData;
        }

        private List<SelectListItem> CreateJewelryCategory(JewelryCategory JewelryCategory, int? newsCategoryId, string prefix = "")
        {
            var lstData = new List<SelectListItem>();
            lstData.Add(new SelectListItem { Value = JewelryCategory.Id.ToString(), Text = prefix + JewelryCategory.Name });
            foreach (var item in JewelryCategory.Childrens)
            {
                if (newsCategoryId.HasValue && !item.Id.Equals(newsCategoryId.Value))
                {
                    lstData.AddRange(CreateJewelryCategory(item, newsCategoryId, prefix + "---"));
                }
        ...
        [HttpPost]
        [Route("Create")]
        public ActionResult Create(JewelryModel model)
        {
            if (CheckFunctionPermission(Constants.PERMISSION_JEWELRY_ADD))
            {
                if (ModelState.IsValid)
                {
                    _JewelryService.Add(model);
        ...
        {
            if (CheckFunctionPermission(Constants.PERMISSION_JEWELRY_EDIT))
            {
                var data = _JewelryService.GetById(id);
        ...
        public ActionResult Edit(JewelryModel model)
        {
            if (CheckFunctionPermission(Constants.PERMISSION_JEWELRY_EDIT))
            {
                if (ModelState.IsValid)
                {
                    var result = _JewelryService.Update(model);
                    if (result.IsSuccess())
                    {
        ...
        [HttpPost]
        [Route("EditExcel")]
        public ActionResult EditExcel(JewelryModel model, int idExcel)
        {
            if (CheckFunctionPermission(Constants.PERMISSION_JEWELRY_EDIT))
            {
                if (ModelState.IsValid)
                {
                    model.Id = 0;
                    var result = _JewelryService.Add(model);
                    if (result.IsSuccess())
                    {
                        var excel = new GenericExcelHandler<Jewelry>(Log, _hostEnvironment.ContentRootPath, UserData.Id);
                        excel.Read();
        ...
        {
            MessageResult message = new MessageResult();
            if (CheckFunctionPermission(Constants.PERMISSION_JEWELRY_DELETE))
            {
                message = new MessageResult();
                message = _JewelryService.Delete(id);
