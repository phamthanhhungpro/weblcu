...
{
    [Route("hoat-dong-du-lich")]
    [Route("travel")]
    public class FETravelController : BaseController
    {
        private TravelService _TravelService;
        public FETravelController(TravelService TravelService, PeopleCategoryService peopleCategoryService, SettingUtils settingUtils) : base(peopleCategoryService, settingUtils)
        {
        ...
        {
            var data = _TravelService.GetFirstOrDefault(o => o.Url.Equals(name) && o.Status == Enums.ActiveStatus.Active, "People");
            if (data != null)
            {
                _TravelService.AddView(data.Id);
                ViewBag.OtherTravel = GetOtherTravel(data);
        ...
        private List<TravelModel> GetOtherTravel(Travel national)
        {
            var data = new List<TravelModel>();
